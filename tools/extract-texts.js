#!/usr/bin/env node
/* Step 1 of the audio pipeline: collect every German text the page can play,
...
/* start page pronunciation table, verb list, survival phrases */
const page = fs.readFileSync(path.join(ROOT, 'src', 'page.html'), 'utf8');
const startTpl = (page.match(/<template id="tpl-start">([\s\S]*?)<\/template>/) || [])[1] || '';
addFromHtml('extra', startTpl);
